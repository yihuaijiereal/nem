...
namespace rdma{
    static const uint32_t maxInlineSize = 0;
        ...
        , completionQueuePair(completionQueuePair)
    {
        ...
        queuePairAttributes.cap.max_send_wr = 8000;                    // Requested max number of outstanding WRs in the SQ
        queuePairAttributes.cap.max_recv_wr = 8000;                    // Requested max number of outstanding WRs in the RQ
    ...
    void QueuePair::connect(const Address &address, unsigned retryCount)
    {
        ...
        wire_gid_to_gid(address.gid,&dgid);
        memset(&attributes, 0, sizeof(attributes));
        attributes.qp_state = IBV_QPS_RTR;
        attributes.path_mtu = IBV_MTU_1024;             // Maximum payload size
        ...
        attributes.ah_attr.is_global = 1;               // Whether there is a global routing header
        //attributes.ah_attr.dlid = address.lid;          // The LID of the remote host
        attributes.ah_attr.grh.dgid = dgid;
        ...
        attributes.max_rd_atomic = 0;     // The number of outstanding RDMA reads & atomic operations (initiator)
    ...
    void QueuePair::postWorkRequest(const WorkRequest &workRequest)
    {
        ...
        if (status != 0) {
            ...
            cerr << reason << workRequest.wr.get()->opcode <<endl;
            throw NetworkException(reason);
        }
    }
