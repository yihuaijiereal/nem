...
namespace rdma{
        ...
        : ibport(1)
    {
        ...
        int i=0;
        for (i = 0; devices[i]; ++i)
            if (!strcmp(ibv_get_device_name(devices[i]),
                    devname))
                break;
        ...
        std::cout<<::ibv_get_device_name(devices[0])<<" opened."<<std::endl;
    ...
    //---------------------------------------------------------------------------
    Network::~Network(){
        auto rq = sharedReceiveQueue.release();
        delete rq;
        auto cqp = sharedCompletionQueuePair.release();
        delete cqp;

        // Deallocate the protection domain
        auto status = ::ibv_dealloc_pd(protectionDomain);
        if (status != 0) {
            string reason =
                    "deallocating the protection domain failed with error " + to_string(errno) + ": " + strerror(errno);
            cerr << reason << endl;
            throw NetworkException(reason);
        }
    ...
    //---------------------------------------------------------------------------
    void Network::getsgid(Address &address){
        union ibv_gid sgid;

        if(ibv_query_gid(context,ibport,0,&sgid))
        {
            string reason = "querying gid " + to_string(ibport) + " failed with error " + to_string(errno) + ": " + strerror(errno);
            cerr << reason << endl;
            throw NetworkException(reason);
        }

        gid_to_wire_gid(&sgid,address.gid);
    }
